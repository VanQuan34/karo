...
if(isset($smof_data['ftc_prod_advanced_zoom']) && $smof_data['ftc_prod_advanced_zoom'] == ''){
	?>
	<div class="details-img <?php if($vertical_thumbnail){ echo 'vertical' ;} ?>">
		...
		if ( has_post_thumbnail() ) {
			...
			$attributes = array(
				...
				'xoriginal'               => get_the_post_thumbnail_url( $post->ID, 'full'),
				'id'                      => 'xzoom-magnific',
			);

			if( isset($smof_data['ftc_prod_cloudzoom']) && $smof_data['ftc_prod_cloudzoom'] == 1 ){
				$html  = '<div data-thumb="' . get_the_post_thumbnail_url( $post->ID) . '" class="ftc-main-images woocommerce-product-gallery__image"><a href="' . esc_url( $full_size_image[0] ) . '" class="woocommerce-main-image swipebox cloud-zoom zoom '.(wp_is_mobile()?'':'on_pc').'" id=\'product_zoom\' data-rel="position:\'inside\',showTitle:0,titleOpacity:0.5,lensOpacity:0.5,fixWidth:362,fixThumbWidth:72,fixThumbHeight:72,adjustX: 0, adjustY:'.(wp_is_mobile()?'0':'-4').'">';
				$html .= get_the_post_thumbnail( $post->ID, 'shop_single', $attributes );
				$html .= '</a>';
				$html .= '</div>';
				$html .='<div  class="ftc_lightbox" >';
				$html .= '<a href="' . esc_url( $full_size_image[0] ) . '" class="swipebox" title="'.esc_html($title).'">';
				$html .= '<span class="fullscreen">'.esc_html__('Fullscreen', 'karo').'';
				$html .= '</span>';
				$html .= get_the_post_thumbnail( $post->ID, 'shop_single', $attributes );	
				$html .='</a></div>';
			}
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'karo' ) );
			$html .= '</a></div>';
		}
