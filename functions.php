...
add_action('wp_footer' , 'popup_demoall');
function popup_demoall(){
    echo '<div class="demo-all">';
    echo '<h2 style="text-align:center;margin-bottom: 50px;">Karo Jewellery Demos</h2>';
    echo '<span class="close">Close</span>';
    echo '</div>';
}


add_action('wp_ajax_nopriv_all_post_demo', 'all_post_demo');
add_action('wp_ajax_all_post_demo', 'all_post_demo');
function all_post_demo(){
    ?>
            ...
            <div class="col-sm-3">
             <a href="<?php echo esc_url(the_permalink()); ?>">
              <?php  if(has_post_thumbnail()){
               echo get_the_post_thumbnail($pages->ID, 'thumbnail'); 
           }
           else{
            echo '<img title="noimage" src="'.get_template_directory_uri().'/assets/images/no-image-blog.jpg" alt="" />';
        }
        ?>
        <h5><?php the_title(); ?></h5>
    </a>
...
// $childpages = wp_list_pages( 'sort_column=menu_order&title_li=&child_of=' . $page->ID . '&echo=0' );
    $childpages = get_pages('sort_column=menu_order&title_li=&child_of=' . $page->ID . '&echo=0&number=1');
    ...
    if($childpages){
        ...
        foreach($childpages as $pages){   
            ?>
            <li>
                <?php echo get_the_post_thumbnail($pages->ID, 'thumbnail'); ?>
                <h4><a href="<?php echo get_page_link($pages->ID) ?>"><?php echo $pages->post_title ?></a></h4>
            </li>
            <?php
        }

        echo '</ul>';
        echo '<button class="load-more-page">Load more</button>';
    } 

}

add_shortcode('wpb_childpages', 'wpb_list_child_pages');
add_shortcode('wpb_childpages2', 'wpb_list_child_pages_2');
...
function more_post_ajax(){
    ...
    if ($loop -> have_posts()) :  while ($loop -> have_posts()) : $loop -> the_post();
        ...
        $out .= '<a href="'.get_page_link($pages->ID).'">';
        $out .= !empty(has_post_thumbnail()) ? get_the_post_thumbnail($pages->ID, 'thumbnail') : '<img title="noimage" src="'.get_template_directory_uri().'/assets/images/no-image-blog.jpg" alt="" />';
...
function wpb_list_child_pages_2(){
    ?>
    <div id="ajax-posts" class="row">
        <?php
        $postsPerPage = 8;
...
<div id="more_posts">Load More</div> <?php echo $total; ?>
<div id="load_all"><?php echo esc_html__('Load all demos '.'('. esc_attr($total).')'); ?></div>
<?php
}
...
add_filter('body_class',function($classes){
        return array_merge($classes, array('megamenu_enabled'));
    });

add_action('init','register_multimenu');
function register_multimenu(){
 register_nav_menus(array(
    'ftc_menu_mobile'=>__('Menu mobile'),
    'ftc_categories_mobile'=>__('Categories mobile')
));
}
            ...
            <div class="ftc-mobile-wrapper">
                ...
                <div class="mobile-menu-wrapper">
                    <ul class="tab-menu-mobile">
                        <?php 
                        if ( has_nav_menu( 'ftc_menu_mobile' ) ) {
                            echo '<li class="menu-mobile menu-active">'.esc_html__('MENU' , 'karo').'</li>';
                        }
                        if ( has_nav_menu( 'ftc_categories_mobile' ) ) {
                           echo '<li class="menu-cate-mobile">'.esc_html__('CATEGORIES' , 'karo').'</li>';
                       }
                   ...
                   <?php
                   if ( has_nav_menu( 'ftc_menu_mobile' ) ){
                    echo '<div class="ftc-menu-mobile menu-active">';
                    wp_nav_menu( array( 'theme_location' => 'ftc_menu_mobile'  , 'menu_id' => 'main-menu', 'menu_class' => 'ftc-smartmenu ftc-simple') );
...
if ( has_nav_menu( 'ftc_categories_mobile' ) ){
    echo '<div class="ftc-categories-mobile">';
        wp_nav_menu( array( 'theme_location' => 'ftc_categories_mobile'  , 'menu_id' => 'main-menu', 'menu_class' => 'ftc-smartmenu ftc-simple') );
    echo '</div>';
}
...
if (!function_exists('ftc_theme_logo')) {
    ...
    function ftc_theme_logo() {
        ...
        $logo_text = isset($smof_data['ftc_text_logo']) ? stripslashes(esc_attr($smof_data['ftc_text_logo'])) : '';
        $attachment_ids = pippin_get_image_id(  esc_url($smof_data['ftc_logo'] ['url']) );
        ...
        <div class="logo">
            <a href="<?php echo esc_url(home_url('/')); ?>">
                <!-- Main logo -->
                <?php if (strlen($logo_image) > 0): ?>
                    <img src="<?php echo esc_url($logo_image); ?>" width="<?php echo esc_attr($w); ?>" height="<?php echo esc_attr($h); ?>" alt="<?php echo!empty($logo_text) ? esc_attr($logo_text) : get_bloginfo('name'); ?>" title="<?php echo!empty($logo_text) ? esc_attr($logo_text) : get_bloginfo('name'); ?>" class="normal-logo" />
                <?php endif; ?>
