...
function ftc_template_redirect(){
	...
	if( is_page() || is_tax( get_object_taxonomies( 'product' ) ) || is_post_type_archive('product') ){
		...
		$page_option_default = array(
			...
			,'ftc_body_font_family'		=> ''
			,'ftc_body_font_enable_google_font'		=> 'default'
			,'ftc_layout_style'		=> 'default'
			,'ftc_dark_layout'							=> 0
			,'ftc_box_layout'							=> 0
			, 'ftc_page_revo_slider'                    => ''
		...
		if( $ftc_page_datas['ftc_layout_style'] != 'default' ){
			$smof_data['ftc_layout_style'] = $ftc_page_datas['ftc_layout_style'];
		}
		...
		if( $ftc_page_datas['ftc_body_font_enable_google_font'] != 'default' ){
			$smof_data['ftc_body_font_enable_google_font'] = $ftc_page_datas['ftc_body_font_enable_google_font'];
		}
		...
		if( $ftc_page_datas['ftc_full_page'] ){
			add_filter( 'body_class', function( $classes ) {
				return array_merge( $classes, array('full-page') );
			} );
		}
		if( $ftc_page_datas['ftc_dark_layout'] ){
			
			add_filter( 'body_class', function( $classes ) {
				return array_merge( $classes, array('dark_layout') );
			} );
		}
		if( $ftc_page_datas['ftc_box_layout'] ){
			
			add_filter( 'body_class', function( $classes ) {
				return array_merge( $classes, array('box_layout') );
			} );
		}
	...
	if( is_singular('product') ){
		...
		foreach ($terms as $term) {
			...
			$bg_breadcrumbs_id = get_term_meta($product_cat_id, 'bg_breadcrumbs_id', true);
		if( $bg_breadcrumbs_id != '' ){
			$bg_breadcrumbs_src = wp_get_attachment_url( $bg_breadcrumbs_id );
			if( $bg_breadcrumbs_src !== false ){
				$smof_data['ftc_bg_breadcrumbs'] ['url'] = $bg_breadcrumbs_src;
...
function ftc_remove_hooks_from_shop_loop(){
	...
	if( ! $smof_data['ftc_prod_cat_cat'] ){
		remove_action('woocommerce_after_shop_loop_item', 'ftc_template_loop_categories', 21);
	}
