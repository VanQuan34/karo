...
switch( true ){
	case is_day():
		$page_title = esc_html__( 'Day: ', 'karo' ) . get_the_date();
	break;
	case is_month():
		$page_title = esc_html__( 'Month: ', 'karo' ) . get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'karo' ) );
	break;
	case is_year():
		$page_title = esc_html__( 'Year: ', 'karo' ) . get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'karo' ) );
	break;
	case is_search():
		$page_title = esc_html__( 'Search Results for: ', 'karo' ) . get_search_query();
	break;
	case is_tag():
		$page_title = esc_html__( 'Tag: ', 'karo' ) . single_tag_title( '', false );
	break;
	case is_category():
		$page_title = esc_html__( 'Category: ', 'karo' ) . single_cat_title( '', false );
	break;
	case is_404():
		$page_title = esc_html__( 'OOPS! FILE NOT FOUND', 'karo' );
	break;
	default:
		$page_title = esc_html__( 'Archives', 'karo' );
	break;
}
